...
// prototype_v10.js — Bug #11 + #14 fixed
...
// Bug #11 fix: LAND + DROWN triggers wired
// Bug #14 fix: proximityThreshold = 2u (spec-correct)
  ...
  try{
    ...
    ae.onload=function(){
      if(window.audioEngine){
        ...
        if(audioEngine.setProximityTarget){audioEngine.setProximityTarget(player,crystals,2);}
      }
    };
...
function triggerDeath(){
  ...
  deaths++;updateDOM();deathTimer=1.5;
...
function triggerDrown(){
  if(deathTimer>0)return;
  deaths++;updateDOM();deathTimer=1.5;
  if(audioEngine)audioEngine.trigger('DROWN');
}
...
function updatePlayer(dt){
  ...
  for(var i=0;i<allPlats.length;i++){
    ...
    if(side==='top'){
      player.y=allPlats[i].y-player.h;player.vy=0;player.grounded=true;
      if(!wasGrounded&&audioEngine)audioEngine.trigger('LAND');
    }
  ...
  // DROWN: fall below level
  if(player.y>H){triggerDrown();return;}

  // Spikes (only above ceiling line)
  if(player.y<H-32){
  ...
  if(audioEngine&&audioEngine.updateProximitySound){audioEngine.updateProximitySound(player,crystals,2);}
}
...
function drawMovingPlatforms(){
  for(var i=0;i<movingPlatforms.length;i++){
    ...
    var tri=mp.type==='movingH'?mp.currentVx>0?[[cx-6,cy+4],[cx+6,cy+4],[cx,cy-4]]:[[cx-6,cy-4],[cx+6,cy-4],[cx,cy+4]]:[[cx,cy-5],[cx-5,cy+5],[cx+5,cy+5]];
...
function drawPlayer(){
  if(deathTimer>0){
    ...
    ctx.globalAlpha=alpha;ctx.fillStyle='#ff4757';ctx.fillRect(player.x,player.y,player.w,player.h);ctx.globalAlpha=1;return;
  }
  ctx.fillStyle='#00d4ff';ctx.fillRect(player.x,player.y,player.w,player.h);
}
function drawHUD(){
  var fps=lastTime>0?Math.round(1/lastTime):0;
  ctx.fillStyle='rgba(0,0,0,0.5)';ctx.fillRect(4,4,120,88);
  ctx.fillStyle='#fff';ctx.font='12px monospace';ctx.textAlign='left';
  ctx.fillText('FPS: '+fps,10,18);
  ctx.fillStyle='#ffd700';ctx.fillText('CRYSTALS: '+collected+'/'+crystalGate,10,36);
...
function gameLoop(timestamp){
  var dt=Math.min((timestamp-lastTime)/1000,0.05);lastTime=timestamp;animTime+=dt;
  updatePlayer(dt);updateMovingPlatforms(dt);updateTimedPlatform(dt);render();
  requestAnimationFrame(gameLoop);
}

preloadTiles();initAudio();requestAnimationFrame(gameLoop);
