/**
 * Dungeon Runner — Physics Prototype v0.3 "Vertical Slice"
 * Zephyr | Game Developer | 2026-04-23
 * 
 * INTEGRATION: Kairo's 23-frame Orb sprites + Cedar's 29 tile PNGs
 * 
 * Sprite sources:
 * - Player: docs/art/sprites/player_{state}_{n}.png (32x32)
 * - Tiles: docs/art/tilesets/level_1/platforms/*.png (32x32)
 * - Props: docs/art/tilesets/level_1/props/*.png
 * - Crystals: docs/art/tilesets/level_1/collectibles/crystal.png
 */
...
function updateLoadingProgress() {
  if (!loadingEl) {
    ...
    if (!loadingEl) {
      ...
      loadingEl.style.cssText = 'position:fixed;top:50%;left:50%;transform:translate(-50%,-50%);color:#00d4ff;font-family:Courier New;font-size:14px;';
...
Object.entries(LEVEL).forEach(([k, v]) => {
  if (Array.isArray(v)) {
    v.forEach(item => {
      if (item.tile) {
        const img = new Image();
        img.src = `${BASE}/docs/art/tilesets/level_1/${item.tile}`;
        img.onerror = () => {};
        levelImages[item.tile] = img;
      }
    });
  }
});
...
function render() {
  ...
  // Platforms (with tile images from Cedar)
  for (const p of platforms) {
    const img = levelImages[p.tile];
    if (img && img.complete && img.naturalWidth > 0) {
      ctx.drawImage(img, p.x, p.y, p.w, p.h);
    } else {
      // Fallback to palette color
  ...
  // Exit portal (if all crystals collected)
  ...
  // Player — sprite with squash/stretch
  ...
  if (loadedSprites < totalSprites) {
    ...
    ctx.fillText(`Loading sprites ${loadedSprites}/${totalSprites}...`, W/2, H/2);
  }
  ...
  if (levelComplete) {
    ...
    ctx.fillStyle = '#888';
    ctx.fillText('Audio: SFX_LEVEL_COMPLETE → Cadenza', W/2, H/2 + 50);
  }
}
...
function gameLoop(timestamp) {
  ...
  animTimer += dt;
  if (animTimer >= 0.12) { // ~8fps animation
    animFrame++;
    animTimer = 0;
  }
