...
function initLevelFromJSON(data){
  ...
  if(data.movingPlatforms)data.movingPlatforms.forEach(function(mp){
    ...
    movingPlatforms.push({x:mpx,y:mpy,w:mp.w*T||96,h:mp.h*T||16,tileKey:mp.tileKey||"platforms/platform_mp_h_00.png",dir:1,baseX:mpx,baseY:mpy,minX:Math.min(mpd,mpd2),maxX:Math.max(mpd,mpd2),minY:Math.min(mpdY,mpd2Y),maxY:Math.max(mpdY,mpd2Y),velX:mp.velocityX||spd,velY:mp.velocityY||0,type:mp.type||"horizontal",warning:false,warned:false});
  });
