...
function initAudio(){var ae=document.createElement("script");ae.src="https://forge-game-dev.github.io/ninfa-engine/docs/audio/audio_engine.js";ae.onload=function(){audioEngine=window.audioEngine;if(audioEngine&&audioEngine.startProximitySound){audioEngine.startProximitySound();}};ae.onerror=function(){console.warn("Audio engine not loaded");};document.head.appendChild(ae);}
...
function restartFromCheckpoint(){if(lastCheckpoint){player.x=lastCheckpoint.x;player.y=lastCheckpoint.y;player.vx=0;player.vy=0;player.grounded=false;}deathTimer=0;levelComplete=false;}
  if(audioEngine&&audioEngine.setProximityTarget){audioEngine.setProximityTarget(player,crystals);}
...
function render(){
  ...
  ctx.fillText("Crystals: "+collected+" / "+crystalGate,8,H-30);
  ctx.fillText("Deaths: "+deaths,8,H-14);
