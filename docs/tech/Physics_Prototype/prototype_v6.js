...
// Moving platforms, water zones, checkpoints, spike hazards
// Audio: Cadenza AudioEngine v0.5
const W = 800, H = 600;
let player, platforms, crystals, torches, checkpoints, hazards, waterZones, exitPortal, levelComplete;
let movingPlatforms = [];
...
function createPlayer(x, y) {
  return { x, y, w:32, h:32, vx:0, vy:0, grounded:false, coyote:0, jumpBuffer:0, facingRight:true,
    state:'idle', animTime:0, animFrame:0, squashX:1, squashY:1, health:1, dying:false };
}
...
function updatePlayer(dt) {
  ...
  if ((keys['Space']||keys['ArrowUp']||keys['KeyW']) && p.jumpBuffer <= 0 && p.coyote > 0) {
    p.vy = -PHYSICS.jumpForce; p.coyote = 0; p.jumpBuffer = 0; p.grounded = false;
    if (audioEngine) audioEngine.trigger('JUMP');
  } else if ((keys['Space']||keys['ArrowUp']||keys['KeyW']) && p.jumpBuffer <= 0 && p.coyote > 0) {
    // coyote jump
    p.vy = -PHYSICS.jumpForce; p.coyote = 0; p.jumpBuffer = 0; p.grounded = false;
    if (audioEngine) audioEngine.trigger('COYOTE_JUMP');
  }
  ...
  p.grounded = false;
  for (const pl of platforms) { resolveSolid(p, pl); }
  for (const mp of movingPlatforms) { resolveSolid(p, mp); }
  for (const hz of hazards) { if (aabb(p, hz)) { triggerDeath(); return; } }
  for (const wz of waterZones) { if (aabb(p, wz)) { triggerDeath(); return; } }

  for (const c of crystals) {
    if (!c.collected && aabb(p, {x:c.x, y:c.y, w:24, h:24})) {
      c.collected = true; if (audioEngine) audioEngine.trigger('CRYSTAL_COLLECT');
    }
  }
  for (const t of torches) {
    if (!t.collected && aabb(p, {x:t.x, y:t.y, w:24, h:24})) {
      t.collected = true; if (audioEngine) audioEngine.trigger('CHECKPOINT');
    }
  }
  for (const cp of checkpoints) {
    if (!cp.activated && aabb(p, {x:cp.triggerX-16, y:cp.triggerY-32, w:48, h:48})) {
      cp.activated = true; lastCheckpoint = cp; if (audioEngine) audioEngine.trigger('CHECKPOINT');
    }
  }
  if (exitPortal && crystals.every(c => c.collected) && aabb(p, exitPortal)) {
    levelComplete = true; if (audioEngine) audioEngine.trigger('LEVEL_COMPLETE');
  }
...
function resolveSolid(p, rect) {
  ...
  const ox = p.x+p.w/2-(rect.x+rect.w/2), oy = p.y+p.h/2-(rect.y+rect.h/2);
  const dx = p.w/2+rect.w/2-Math.abs(ox), dy = p.h/2+rect.h/2-Math.abs(oy);
  if (dx < dy) {
    if (ox < 0) { p.x = rect.x - p.w; p.vx = 0; } else { p.x = rect.x+rect.w; p.vx = 0; }
  } else {
    if (oy < 0) {
      p.y = rect.y - p.h; p.vy = 0; p.grounded = true;
      if (rect.type && rect.type.startsWith('moving')) { rect.wasOn = true; }
    } else { p.y = rect.y+rect.h; p.vy = 0; }
  }
}

function resolvePassthrough(p, rect) {
  if (!aabb(p, rect)) return;
  const prevBottom = p.y + p.h - p.vy * 0.017;
  const inX = p.x < rect.x+rect.w && p.x+p.w > rect.x;
  const crossTop = prevBottom <= rect.y && (p.y+p.h) > rect.y && (p.y+p.h) <= rect.y+12;
  if (inX && crossTop && p.vy >= 0) { p.y = rect.y - p.h; p.vy = 0; p.grounded = true; }
}

function triggerDeath() {
  if (player.dying) return;
  player.dying = true; deathTimer = 0.5;
  if (audioEngine) audioEngine.trigger('DEATH');
  player.state = 'death'; player.animTime = 0; player.animFrame = 0;
}

function respawnAtCheckpoint() {
  const spawn = lastCheckpoint ? {x: lastCheckpoint.x, y: lastCheckpoint.y} : {x:32, y:64};
  player = createPlayer(spawn.x, spawn.y);
}

function advanceAnim(state, dt) {
  ...
  if (p.animTime >= dur) {
    ...
    if (p.animFrame >= frames.length) {
      if (SPRITE_LOOPS[state]) p.animFrame = 0;
      else p.animFrame = frames.length - 1;
    }
  }
}
...
function drawPlayer() {
  const p = player;
  if (!p) return;
  const sq = SQUASH_MAP[p.state] || {sx:1,sy:1};
  const sw = p.w * sq.sx, sh = p.h / sq.sy;
  const ox = (p.w - sw) / 2, oy = p.h - sh;
  const frames = SPRITE_MAP[p.state] || SPRITE_MAP.idle;
  const img = spriteImgs[p.state] && spriteImgs[p.state][p.animFrame];
  ctx.save();
  if (!p.facingRight) { ctx.translate(p.x + p.w/2, 0); ctx.scale(-1, 1); ctx.translate(-(p.x + p.w/2), 0); }
  ...
  ctx.restore();
  const ga = GLOW_ALPHA[p.state] || 0.05;
  if (ga > 0) {
    ctx.fillStyle = `rgba(0,212,255,${ga})`; ctx.beginPath(); ctx.arc(p.x+p.w/2, p.y+p.h/2, p.w * 0.85, 0, Math.PI*2); ctx.fill();
  }
}
...
function initLevel() {
  LEVEL = {
    spawn:{x:32, y:64},
    crystals:[
      {id:'C-01',x:96,y:160},{id:'C-02',x:224,y:192},{id:'C-03',x:384,y:160},
      {id:'C-04',x:160,y:256},{id:'C-05',x:384,y:256},{id:'C-06',x:448,y:288},
      {id:'C-07',x:64,y:352},{id:'C-08',x:224,y:352},{id:'C-09',x:320,y:416},
      {id:'C-10',x:480,y:416},{id:'C-11',x:128,y:480},{id:'C-12',x:384,y:544}
    ],
    torches:[
      {x:64,y:160},{x:224,y:192},{x:352,y:160},{x:96,y:256},
      {x:448,y:288},{x:32,y:352},{x:256,y:544}
    ],
    exit:{x:480,y:544,w:48,h:64}
  };
  ...
  for (let y=0; y<18; y++) {
    for (let x=0; x<25; x++) {
      ...
      if (tile === 'X') platforms.push({x:x*32,y:y*32,w:32,h:32,type:'solid'});
      if (tile === 'P') platforms.push({x:x*32,y:y*32,w:32,h:32,type:'passthrough'});
    }
  }
  ...
  movingPlatforms = [
    {x:128,y:160,w:64,h:16,type:'movingH',velocityX:3,currentVx:3,currentVy:0,minX:128,maxX:220,wasOn:false},
    {x:320,y:256,w:64,h:16,type:'movingV',velocityY:2,currentVx:0,currentVy:2,minY:192,maxY:352,wasOn:false}
  ];
...
function gameLoop(timestamp) {
  ...
  if (el('cr')) el('cr').textContent = crystals ? crystals.filter(c=>c.collected).length : 0;
...
function render() {
  ...
  if (crystals && crystals.every(c=>c.collected)) {
...
function drawCrystals() {
  ...
  for (const c of crystals) {
    ...
    const glow = 0.15 + 0.1 * Math.sin(t * 3 + c.x * 0.1);
    ctx.fillStyle = `rgba(255,215,0,${glow})`; ctx.beginPath(); ctx.arc(c.x+12, c.y+12, 20, 0, Math.PI*2); ctx.fill();
    ctx.fillStyle = '#ffd700'; ctx.shadowColor = '#ffd700'; ctx.shadowBlur = 8;
    ctx.beginPath(); ctx.moveTo(c.x+12, c.y); ctx.lineTo(c.x+20, c.y+10); ctx.lineTo(c.x+16, c.y+24); ctx.lineTo(c.x+8, c.y+24); ctx.lineTo(c.x+4, c.y+10); ctx.closePath(); ctx.fill();
    ctx.shadowBlur = 0;
  }
}
